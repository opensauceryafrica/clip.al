...
async function updateCounters(perLink: Map<string, { delta: number; maxTs: Date }>): Promise<void> {
  // Keyed by link_id (not code) so clicks on a renamed/old back-half still credit
  // the link. One UPDATE per link per batch — cheap even at high throughput (§10).
  for (const [linkId, { delta, maxTs }] of perLink) {
    await db
      .update(links)
      .set({
...
async function processEntries(entries: StreamEntry[]): Promise<void> {
  ...
  for (const [id, fieldArr] of entries) {
    try {
      ...
      if (!ua.isBot && linkId) {
        const cur = perLink.get(linkId) ?? { delta: 0, maxTs: tsDate };
        cur.delta += 1;
